...
type Store struct {
	...
	recov *recovlog.Log // The optimized recovery log for raft
}
...
func (s *Store) Get(key string) (string, error) {
	...
	s.recov.Put(0, recovlog.Read, str, time.Now().Format(time.Stamp))
...
func (s *Store) Set(key, value string) error {
	...
	if f.Error() == nil {
		...
		s.recov.Put(0, recovlog.Write, str, time.Now().Format(time.Stamp))
	}
